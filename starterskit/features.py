...
def extractDAISY(input_image, corner_coords, radius=16, rings=3):
    ...
    # unused dummy value
    keypoints = [cv2.KeyPoint(float(x), float(y), size=-1) for coords in corner_coords for (x, y) in coords]
    gray = cv2.cvtColor(input_image, cv2.COLOR_BGR2GRAY)
...
def extractFREAKCallback(input_image):
    ...
    keypoints = [cv2.KeyPoint(float(x), float(y), size=-1) for coords in corners for (x, y) in coords]
